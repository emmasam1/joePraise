...
//                   <label className="text-[#060853] font-bold text-xs">
//                     Name
//                   </label>
//                   <Input
//                     placeholder="Name"
...
//                     <p
//                       className={`text-[10px] flex items-center gap-1 transition-colors ${validations.length ? "text-[#15BE87]" : "text-gray-400"}`}
//                     >
//                       {validations.length ? (
...
//                     </p>
//                     <p
//                       className={`text-[10px] flex items-center gap-1 transition-colors ${validations.numberOrSymbol ? "text-[#15BE87]" : "text-gray-400"}`}
//                     >
//                       {validations.numberOrSymbol ? (
//                         <CheckCircleFilled className="text-[9px]" />
//                       ) : (
...
//                     </p>
//                     <p
//                       className={`text-[10px] flex items-center gap-1 transition-colors ${validations.case ? "text-[#15BE87]" : "text-gray-400"}`}
//                     >
//                       {validations.case ? (
//                         <CheckCircleFilled className="text-[9px]" />
//                       ) : (
...
const SignUpPage = () => {
  ...
  const handleGoogleSignup = () => {
    ...
    const stateParam = Object.keys(statePayload).length
      ? `?state=${encodeURIComponent(JSON.stringify(statePayload))}`
      : "";

    window.location.href = `${baseUrl}/auth/google${stateParam}`;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        message.success(response.data.message || "Verification code sent!");
        // NEW: carry redirect forward to the OTP page
        const otpUrl = `/verification-code?email=${encodeURIComponent(formData.email)}${redirectTo ? `&redirect=${encodeURIComponent(redirectTo)}` : ""}`;
        router.push(otpUrl);
      }
    } catch (error) {
      message.error(
