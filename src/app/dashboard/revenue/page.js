...
const DEFAULT_RANGE_LABEL = "May 1, 2026 - Jun 22, 2026";
...
const getStatusColor = (status) => STATUS_COLOR_MAP[status] || "#94A3B8";
...
const page = () => {
  ...
  const handleStartDateChange = (date) => {
    setCustomStart(date);
    setCurrentPage(1);
    fetchWithFilters({ start: date, end: customEnd, targetPage: 1 });
  };

  const handleEndDateChange = (date) => {
    setCustomEnd(date);
    setCurrentPage(1);
    fetchWithFilters({ end: date, start: customStart, targetPage: 1 });
  };

  const handleServiceTypeChange = (value) => {
    setServiceType(value);
    setCurrentPage(1);
    fetchWithFilters({ type: value, targetPage: 1 });
  };

  const goToPage = (targetPage) => {
    if (targetPage < 1 || (pagination?.totalPages && targetPage > pagination.totalPages)) return;
    setCurrentPage(targetPage);
    fetchWithFilters({ targetPage });
  };

  // 3. DATA MAPPING (from API, falling back to empty series until loaded)
  const chartDataMap = chartData || { Day: [], Week: [], Month: [], Year: [] };
  ...
  const statsCard = [
    ...
    },
    {
      id: 2,
      title: "Platform Commission Deducted",
      value: `${stats?.commissionPercent ?? 0}%`,
      img: "/images/carbon.png",
    },
    {
      id: 3,
  ...
  const rangeLabel =
    customStart && customEnd
      ? `${customStart.format("MMM D, YYYY")} - ${customEnd.format("MMM D, YYYY")}`
      : DEFAULT_RANGE_LABEL;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid-cols-1 lg:grid-cols-2 gap-20 grid">
        <WeeklyRevenueCard data={weeklyRevenue} />
        <MonthlyRevenueCard data={statusBreakdown} averageValue={averageOrderValueK || 0} />
      </div>
