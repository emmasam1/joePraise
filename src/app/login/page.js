...
const LoginPage = () => {
  ...
  const handleGoogleLogin = () => {
    const baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL.replace('/v1', '');
    // NEW: pass redirect through Google's `state` param so it survives
    // the full-page OAuth round-trip.
    const stateParam = redirectTo ? `?state=${encodeURIComponent(JSON.stringify({ redirect: redirectTo }))}` : "";
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // NEW: honor redirect if present, otherwise fall back to role-based routing
        if (redirectTo) {
          ...
          <form className="flex-grow flex flex-col gap-5 mb-15" onSubmit={handleLogin}>
            ...
            <div className="mt-5 flex flex-col items-center gap-3">
              ...
              <p className="text-center text-gray-600 text-sm mt-4">
                ...
                <Link
                  href={redirectTo ? `/business-registration?redirect=${encodeURIComponent(redirectTo)}` : "/business-registration"}
