...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Category Icon Grid — now live from the API */}
      <section className="py-12 px-6 bg-white">
        <div className="max-w-7xl mx-auto flex flex-wrap justify-center gap-6">
          {publicCategoriesLoading ? (
            [...Array(6)].map((_, i) => (
              <div
                key={i}
                className="w-40 h-44 bg-gray-100 rounded-md animate-pulse"
              />
            ))
          ) : (
            (publicCategories || []).slice(0, 6).map((cat) => (
              ...
              >
                ...
                <div className="w-20 h-20 rounded-full bg-[#F5E6E6] flex items-center justify-center transition-transform group-hover:scale-110 overflow-hidden">
                  {cat.icon ? (
                    <Image src={cat.icon} alt={cat.name} width={40} height={40} unoptimized />
                  ) : (
      ...
      {/* Weekly Newsletter Banner — unchanged, static */}
      <section className="w-full bg-[#060853] py-24 px-6 flex items-center justify-center">
        <div className="w-full max-w-7xl flex flex-col lg:flex-row items-center justify-between gap-12">
          <div className="flex-1">
            <h2 className="text-2xl md:text-3xl font-semibold text-white tracking-tight text-center lg:text-left">
      ...
      {/* Explore Searches — unchanged, static */}
      <section className="py-20 px-6 bg-white font-sans max-w-7xl mx-auto">
        <div className="mb-10">
          <h2 className="text-3xl font-bold text-black mb-2">
            Explore searches in popular cities
