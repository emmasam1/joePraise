    ...
    try {
      if (type === "reset") {
        ...
        if (response.data.success) {
          ...
          // NEW: carry redirect forward to the success page
          const redirectParam = redirectTo ? `&redirect=${encodeURIComponent(redirectTo)}` : "";

          if (user.onboardingStep === "business_personal") {
            router.push(`/verification-successful?type=business_onboarding${redirectParam}`);
          } else {
            router.push(`/verification-successful?type=registration${redirectParam}`);
          }
        }
      }
    } catch (error) {
