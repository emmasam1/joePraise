...
const FALLBACK_DATA = [
  { name: 'Accepted', value: 0, color: '#4f46e5' }, // Purple
  { name: 'In Progress', value: 0, color: '#060853' }, // Navy
  { name: 'Completed', value: 0, color: '#10B981' }, // Emerald
];

const MonthlyRevenueCard = ({ data, averageValue = 0 }) => {
  const donutData = data && data.length ? data : FALLBACK_DATA;
    ...
    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm w-full">
      ...
      <div className="flex items-center gap-10">
        <div className="h-62.5 w-full max-w-62.5 relative">
          ...
          {/* Central Stats Text */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
            <p className="text-3xl font-extrabold text-[#060853]">{averageValue.toFixed(2)}</p>
            <p className="text-xs text-gray-400 mt-1">Average Range</p>
          </div>
        </div>
